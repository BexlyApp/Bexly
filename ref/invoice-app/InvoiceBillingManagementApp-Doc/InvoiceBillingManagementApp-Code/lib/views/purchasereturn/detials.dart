// lib/screens/debit_notes_details_screen.dart (Example path)
...
class DebitNotesDetailsScreen extends StatelessWidget {
  const DebitNotesDetailsScreen({super.key});

  // --- Placeholder Data (Using structure similar to other detail screens) ---
  final Map<String, dynamic> debitNoteHeader = const {
    'title': 'Debit Notes Details',
    'status': 'Paid',
  };
  final Map<String, dynamic> debitNoteInfo = const {
    'date': '15 Mar 2024',
    'debit_note_no': 'QUO - 000015',
  }; // Adjusted key
  ...
  // Using summary from Purchase/Sales Return Details example
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(debitNoteHeader['title']),
        centerTitle: true,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            _buildDebitNoteHeader(debitNoteHeader, statusColor),
            const SizedBox(height: 20),
            _buildDateAndNumberSection(debitNoteInfo),
            const SizedBox(height: 16),
            _buildAddressSection(debitNoteAddresses),
            const SizedBox(height: 20),
            ...
            const SizedBox(height: 20),
            _buildSectionTitle('Summary'), // Changed Title
            _buildDebitNoteSummary(debitNoteSummary),
            const SizedBox(height: 20),
            _buildBottomButtons(),
            const SizedBox(height: 20),
          ],
  ...
  Widget _buildDebitNoteHeader(Map<String, dynamic> header, Color statusColor) {
    return Row(
  ...
  Widget _buildDateAndNumberSection(Map<String, dynamic> info) {
    return Row(
      children: [
        ...
        Expanded(
          child: _buildInfoBox('Debit Note No', info['debit_note_no']),
        ), // Changed label
      ],
    );
  }
  ...
  Widget _buildAddressSection(Map<String, dynamic> addresses) {
    // Assuming same address structure applies
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Expanded(
