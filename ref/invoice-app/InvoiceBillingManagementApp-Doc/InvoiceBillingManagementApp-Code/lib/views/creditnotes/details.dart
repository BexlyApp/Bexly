// lib/screens/credit_notes_details_screen.dart (Example path)
...
class CreditNotesDetailsScreen extends StatelessWidget {
  const CreditNotesDetailsScreen({super.key});

  // --- Placeholder Data ---
  final Map<String, dynamic> creditNoteHeader = const {
    'title': 'Credit Notes Details',
    'status': 'Paid',
  }; // Example
  final Map<String, dynamic> creditNoteInfo = const {
    'date': '15 Mar 2024',
    'credit_note_no': 'QUO - 000015',
  }; // Adjusted key
  final Map<String, dynamic> creditNoteAddresses = const {
    'to_name': 'Naveen Bansel',
    'to_details': 'yodha@gmail.com\n987654321',
    'pay_name': 'KanakkuLLC',
    'pay_details': 'Brooklyn, NY 333\nUSA',
  };
  final List<Map<String, dynamic>> creditNoteItems = const [
    // Same as invoice/sales return for example
    {
      'name': 'Nike Shoe',
      'unit': 'Pc',
      'qty': 10,
  ...
  // Updated Summary based on screenshot ($0 values)
  final Map<String, dynamic> creditNoteSummary = const {
    'amount': 0.00,
    'discount': 0.00,
    'tax': 0.00,
    'total': 0.00,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(creditNoteHeader['title']),
        centerTitle: true,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            _buildCreditNoteHeader(creditNoteHeader, statusColor),
            const SizedBox(height: 20),
            _buildDateAndNumberSection(creditNoteInfo),
            const SizedBox(height: 16),
            _buildAddressSection(creditNoteAddresses),
            const SizedBox(height: 20),
            _buildSectionTitle('Items'),
            _buildItemsList(creditNoteItems),
            const SizedBox(height: 10),
            _buildItemsTotal(
              creditNoteItems.fold(
                0.0,
            ...
            _buildCreditNoteSummary(creditNoteSummary),
            const SizedBox(height: 20),
            _buildBottomButtons(),
            const SizedBox(height: 20),
          ],
  ...
  Widget _buildDateAndNumberSection(Map<String, dynamic> info) {
    return Row(
      children: [
        Expanded(child: _buildInfoBox('Date', info['date'])),
        const SizedBox(width: 16),
        Expanded(
          child: _buildInfoBox('Credit Note No', info['credit_note_no']),
        ),
      ],
    );
  }
  ...
  Widget _buildCreditNoteSummary(Map<String, dynamic> summary) {
    // Displaying values exactly as in screenshot ($0)
    return Container(
      padding: const EdgeInsets.all(16.0),
      decoration: BoxDecoration(
  ...
  Widget _buildBottomButtons() {
    // Reusing from Sales Return Details
    return Row(
      children: [
